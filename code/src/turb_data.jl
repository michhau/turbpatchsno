...
(using col. names 'u','v','w') to set mean(v)=mean(w)=0."
function drdf!(data::DataFrame; periodwise=true, gapthresh=Minute(10))
    endidcs = zeros(Int64, 0)
    if periodwise
        gaps = detectnanandgap(data, gapthresh)
        endidcs = gaps.idx_before_gap
        push!(endidcs, size(data, 1))
        @info("Performing period-wise in-place Double Rotation")
        println(size(gaps, 1) + 1, " periods")
    else
        @info("Performing in-place Double Rotation for one block")
        endidcs = size(data, 1)
    end
    ...
    for per in axes(endidcs, 1)
        if per !== 1
            startidx = gaps.idx_after_gap[per-1]
        end
        endidx = endidcs[per]
...
function interpolatemissing(data::DataFrame, threshgapsize::Int=20)
    ...
    for i in 1:collength-1
        missingarr[:, i] = ismissing.(data[:, i+1])
    end
