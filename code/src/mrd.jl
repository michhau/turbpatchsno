...
#=
function fastnomrdnonan(data_a_in::Vector, data_b_in::Vector, series::Vector{Int64}, nanthresh=0.8)
    data_a = copy(data_a_in)
    data_b = copy(data_b_in)
    timestep = Millisecond(50) #only needed for output
    ...
    for (kidx, k) in enumerate(series)
        ...
        for i in 1:nw
            if i == 1
                idcs = 1:k
                last_sum_a = sum(data_a[idcs])
                last_sum_b = sum(data_b[idcs])
                wmeans_a[1] = last_sum_a / k
                wmeans_b[1] = last_sum_b / k
            else
                idx = k + i - 1
                corrforward_a = data_a[idx]
                corrforward_b = data_b[idx]
                ...
                last_sum_a += (corrforward_a - corrbackward_a)
                last_sum_b += (corrforward_b - corrbackward_b)
                wmeans_a[i] = last_sum_a / k
                wmeans_b[i] = last_sum_b / k
            end
        end

        #write to result vectors
        if kidx > 1
            D[length(series)-kidx+1] = median(filter(!isnan, wmeans_a[1:nw] .* wmeans_b[1:nw]))
        end
