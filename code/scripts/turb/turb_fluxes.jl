######################################################
###      CALCULATE TURBULENT FLUXES/ADVECTION      ###
...
##
######################################################
###     OVERVIEW PLOT WITH METEOROLOGICAL DATA     ###
######################################################
pif = PyPlot.figure()
...
PyPlot.pygui(true)
hig = PyPlot.figure()
axh = hig.add_subplot(111)
axh.set_title(string(evaldf1.time[1], " - ", evaldf1.time[end], "; upvalley wind"))
#axh.set_xlabel(L"\overline{T_{sonic}}~\mathrm{[^\circ C]}")
#axh.set_xlabel(L"\overline{w'\theta'}~\mathrm{[K~m~s^{-1}]}")
axh.set_xlabel(L"\Delta T = T_1 - T_2~\mathrm{[K]}")
#axh.hist(evaldf1night.T, bins=collect(-5:0.01:15), label="T1IRG", alpha=0.5)
#axh.hist(evaldf2night.T, bins=collect(-5:0.01:15), label="T2IRG", alpha=0.5)
#axh.hist(evaldf3night.T, bins=collect(-5:0.01:15), label="T2LCSAT", alpha=0.3)
#axh.hist(evaldf4night.T, bins=collect(-5:0.01:15), label="T2UCSAT", alpha=0.3)
#axh.hist(fx1day.wT, bins=collect(-0.12:0.001:0.1) ,label="T1IRG")
#axh.hist(fx2day.wT, bins=collect(-0.12:0.001:0.1) ,label="T2IRG", alpha=0.5)
#axh.hist(fx3day.wT, bins=collect(-0.12:0.001:0.1) ,label="T2LCSAT", alpha=0.5)
#axh.hist(fx4day.wT, bins=collect(-0.12:0.001:0.1) ,label="T2UCSAT", alpha=0.3)
axh.hist(tmp, bins=collect(-2.5:0.01:2.5))
#axh.legend()
axh.grid()
PyPlot.show()
##